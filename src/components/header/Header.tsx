...
export const Header = () => {
    ...
    const isProjectDetailPage = location.startsWith('/projects/') && location !== '/projects';

    const activeIndex = navItems.findIndex(item => item.path === location);

    const handleMouseMove = (e: React.MouseEvent) => {
        if (!navContainerRef.current || isProjectDetailPage) return;
    ...
    return (
        <header className="main-plate flex flex-row items-center justify-between px-4 sticky top-2 md:top-4 mb-4 md:mb-8 h-16 z-50">
            <div
                ref={navContainerRef} 
                className="flex flex-row gap-4 items-center relative"
                onMouseMove={isProjectDetailPage ? undefined : handleMouseMove}
                onMouseLeave={isProjectDetailPage ? undefined : handleMouseLeave}
            >
                {!isProjectDetailPage && (
                    <div 
                ...
                {navItems.map((item, index) => {
                    ...
                    const isItemActive = !isProjectDetailPage && index === activeIndex;
            ...
            <a 
                ...
                className="md:shadow-hover bg-black text-white font-medium py-2 px-4 text-sm rounded-md flex flex-row gap-1 items-center"
            >
